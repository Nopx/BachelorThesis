...
public class FrameGetter {
	...
	public long getRequiredSpace(String fileString){
		try {
			OpenCVFrameGrabber fg = new OpenCVFrameGrabber(fileString);
			fg.start();
			long fps = (fg.getImageHeight()*fg.getImageWidth()*fg.getBitsPerPixel())/(8*1024*1024);
			fg.stop();
			return fps;
		} catch (Exception e) {
			ErrorHandler.handleTermitatingError(e);
	...
	public ArrayList<Frame> getFrames(File file, int start, int end){
        ...
        OpenCVFrameGrabber fg = new OpenCVFrameGrabber(file);
        int counter = 0;
        Frame f;
        try{
	        fg.start();
