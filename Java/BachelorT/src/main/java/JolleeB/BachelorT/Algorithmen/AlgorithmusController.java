...
public class AlgorithmusController {
	...
	private static final int MEDIANFRAMEAMOUNT =50;
	private static final int SMOOTHINGFRAMEAMOUNT =50;
	...
	private void spatialgmm(){
		SpatialGMMAnalyzer analyzer = new SpatialGMMAnalyzer(3*width,height);
		while(fg.hasFrames()){
			byte[][] framesIn = fg.getNextFrames();
			byte[] frame = framesIn[framesIn.length-1];
			frame = analyzer.convertImage(frame);
