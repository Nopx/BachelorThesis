...
public class MOGAnalyzerMRF {
	...
	public byte[] convertImage(byte[] imgIn){
		...
		float[] highestGaussiansValues = new float[this.imgLength];
		for(int i = 0; i <this.imgLength; i++){
			...
			highestGaussiansValues[i] = sortedGaussians[0][0];
...
//				imgOut[i] = 70;
				...
				if(isBackground) field[i] = 1;
				else field[i] = 0;
			}
		}
		field =mrfOptimizer.optimizeRandomFieldHardAssign(imgIn, field, imgWidth, imgHeight, runs);
		for(int i =0; i < this.imgLength; i++){
			if( field[i] ==0) imgOut[i] = (byte)Math.floor(mean[(int)highestGaussiansValues[i]][i]);
